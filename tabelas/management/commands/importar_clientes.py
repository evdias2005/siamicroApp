...
class Command(BaseCommand):
    def handle(self, *args, **options):
        print('INICIANDO IMPORTAÇÃO DA TABELA DE CLIENTES DO ANTIGO MICROCREDITO =======')
        # print(settings.BASE_DIR)
        caminho_arquivo = os.path.join(settings.BASE_DIR, 'Arquivos/TabelaClientes_v3.csv')
        ...
        for linha in tqdm(linhas[1:]):
            ...
            nome_pai = linha[16].strip()
            nome_mae = linha[17].strip()
            nome_conjuge = linha[18].strip().replace('"', '')
            cpf_conjuge = linha[19].strip().replace('"', '')
            # ponto_referencia = linha[20].strip()                # alguns campos são removidos manualmente pois há erros
            created_at = linha[20].strip()
            updated_at = linha[21].strip()
            logradouro = linha[22].strip()
            # bairro = linha[23].strip()
            cep = linha[24].strip()
            numero = linha[25].strip()
            complemento = linha[26].strip()
            municipio = linha[27].strip()
            uf = linha[28].strip()
            escolaridade = linha[29].strip()
            estadocivil = linha[30].strip()
            nome_empresa = linha[31].strip()
            data_admissao = linha[32].strip().replace('"', '')
            cargo = linha[33].strip()
            renda_liquida = linha[34].strip()
            numero_beneficio = linha[35].strip().replace('"', '')
            rendimentos_alugueis = linha[36].strip()
